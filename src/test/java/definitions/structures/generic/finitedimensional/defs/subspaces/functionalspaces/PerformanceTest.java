...
public class PerformanceTest {
	...
	public static void prepare() throws Throwable {
		...
		exp=new FunctionTuple(
				SpaceGenerator.getInstance().getFiniteDimensionalVectorSpace(1).
						genericBaseToList().get(0).getGenericCoordinates()) {
			@Override
			public double value(double input) {
				double rest=-(Math.exp(Math.PI)-Math.exp(-Math.PI))/(2*Math.PI);
				return Math.exp(input)+rest;
			}
		};
		symExp=new FunctionTuple(
				SpaceGenerator.getInstance().getFiniteDimensionalVectorSpace(1).
						genericBaseToList().get(0).getGenericCoordinates()) {
			@Override
			public double value(double input) {
	...
	public void identity() throws Throwable {	
		Vector toFourier=space.getCoordinates(parabel);
		//((Function)toFourier).plot(-Math.PI,Math.PI);
		parabel.plotCompare(-Math.PI,Math.PI,((Function)toFourier));
		double ans=space.getDistance((Function)toFourier,parabel);
	...
	public void abs() throws Throwable {	
		...
		double ans=space.getDistance((Function)toFourier,abs);
		Assert.assertTrue(ans<0.1);
	}
	
	@Test
	public void exp() throws Throwable {	
		Vector toFourier=space.getCoordinates(exp);
		//((Function)toFourier).plot(-Math.PI,Math.PI);
		exp.plotCompare(-Math.PI,Math.PI,((Function)toFourier));
		double ans=space.getDistance((Function)toFourier,exp);
		Assert.assertTrue(ans<5);
	}
	
	@Test
	public void symExp() throws Throwable {	
		Vector toFourier=space.getCoordinates(symExp);
		//((Function)toFourier).plot(-Math.PI,Math.PI);
		symExp.plotCompare(-Math.PI,Math.PI,((Function)toFourier));
		double ans=space.getDistance((Function)toFourier,symExp);
		Assert.assertTrue(ans<0.1);
	}
	
	@Test
	public void circle() throws Throwable {	
		Vector toFourier=space.getCoordinates(circle);
		//((Function)toFourier).plot(-Math.PI,Math.PI);
		circle.plotCompare(-Math.PI,Math.PI,((Function)toFourier));
		double ans=space.getDistance((Function)toFourier,circle);
		Assert.assertTrue(ans<0.1);
	}
