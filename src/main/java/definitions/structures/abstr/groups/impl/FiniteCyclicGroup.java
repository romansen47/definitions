...
public class FiniteCyclicGroup implements Ring,CyclicGroup {
 
	private static final long serialVersionUID = 1L;

	class Element implements RingElement{
	...
	public FiniteCyclicGroup(int n) {
		setOrder(n);
	...
	public void print() {
		...
		for (int i=0;i<getOrder();i++) {
			...
			for (int j=0;j<getOrder();j++) {
				System.out.print(((FiniteCyclicGroup.Element) operation(get(i),get(j))).getRepresentant()+" ");
			}
			System.out.println();
		}
		...
		for (int i=0;i<getOrder();i++) {
			System.out.print(i+": ");
			for (int j=0;j<getOrder();j++) {
				System.out.print(((FiniteCyclicGroup.Element) getMuliplicativeMonoid().operation(get(i),get(j))).getRepresentant()+" ");
			}
			System.out.println();
		}
	}
