	...
	default Vector addition(final Vector vec1, final Vector vec2) {
		if ((vec1 instanceof FiniteVector) && (vec2 instanceof FiniteVector)) {
			...
			base.stream().forEach(vec -> {
				final double summand1 = ((Real) ((FiniteVector) vec1).getCoordinates().get(this.getBaseVec(vec)))
						.getDoubleValue();
				final double summand2 = ((Real) ((FiniteVector) vec2).getCoordinates().get(this.getBaseVec(vec)))
						.getDoubleValue();
