	...
	default Vector addition(final Vector vec1, final Vector vec2) {
		if ((vec1 instanceof FiniteVector) && (vec2 instanceof FiniteVector)) {
			...
			base.stream().forEach(vec -> {
				final double summand1 = ((FiniteVector) vec1).getCoordinates().get(getBaseVec(vec)).getDoubleValue();
				final double summand2 = ((FiniteVector) vec2).getCoordinates().get(getBaseVec(vec)).getDoubleValue();
